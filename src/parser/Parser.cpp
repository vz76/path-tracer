  ...
  for( ;; )
  {
    ...
    switch( t->kind() )
    {
      ...
      case RBRACE:
        ...
        if (square->getMaterial().Light()) {
            // std::cout << "adding light" << endl;
            isect temp;
            scene->addPathLight(square, square->getMaterial().ke(temp));
        }
        scene->add( square );
        return;
      default:
...
//          mat->setReflective( specular );  // Default kr = ks if none specified
      ...
      case LIGHT: 
        mat->setLight(parseVec3dMaterialParameter(scene));
        break;
