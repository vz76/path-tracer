...
public:
    ...
    Material(const glm::dvec3& e, const glm::dvec3& a, const glm::dvec3& s,
        const glm::dvec3& d, const glm::dvec3& r, const glm::dvec3& t, const glm::dvec3& l, double sh, double in)
        : _ke(e), _ka(a), _ks(s), _kd(d), _kr(r), _kt(t), _kl(l), _light(0),
          _shininess( glm::dvec3(sh,sh,sh) ), _index( glm::dvec3(in,in,in) ) { setBools(); }

    virtual glm::dvec3 shade( Scene *scene, ray& r, const isect& i, double thresh) const;
    ...
    glm::dvec3 kl(const isect& i) const { return _kl.value(i); }
    ...
    void setLight(const glm::dvec3& kl) { 
        _kl.setValue(kl); 
        if (glm::distance(glm::dvec3(), kl) != 0) { // is a light
            _light = true;
        }
    }
    ...
    void setLight(MaterialParameter& kl) { 
        _kl = kl; 
        if (!kl.isZero()) { // is a light
            _light = true;
        }
    }
...
private:
    ...
    MaterialParameter _kl;                    // light indicator vec3
